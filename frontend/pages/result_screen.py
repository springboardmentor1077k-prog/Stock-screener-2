...
def fetch_page(page):
    try:
        ...
        st.error(f"⚠️ API Error ({response.status_code}) while fetching results.")
        return None
...
if total_results == 0 or len(data) == 0:
    st.markdown("""
    <div style="text-align:center;margin-top:80px;color:#9ca3af;font-size:20px;">
        🔍 No companies matched your query.<br><br>
...
with colA:
    st.markdown(
    f"<div class='result-count'>{total_results} companies found • Page {page} of {total_pages}</div>",
    unsafe_allow_html=True
    )
...
def company_card(row):
    ...
    profit_col = profit_color(row["profit_margin"])
    symbol = row["symbol"]
    ...
    with st.container(border=True):
        ...
        with col1:
            st.markdown(f"<div class='company-name'>{row['company_name']}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='company-sub'>{symbol} • {row['sector']}</div>", unsafe_allow_html=True)

        with col2:
            st.markdown("<div class='metric-title'>P/E</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-value'>{round(row['pe_ratio'],2)}</div>", unsafe_allow_html=True)

        with col3:
            st.markdown("<div class='metric-title'>Market Cap</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-value'>{format_number(row['market_cap'])}</div>", unsafe_allow_html=True)

        with col4:
            st.markdown("<div class='metric-title'>Revenue</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-value'>{format_number(row['revenue'])}</div>", unsafe_allow_html=True)

        with col5:
            st.markdown("<div class='metric-title'>EBITDA</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-value'>{format_number(row['ebitda'])}</div>", unsafe_allow_html=True)

        with col6:
            st.markdown("<div class='metric-title'>Profit</div>", unsafe_allow_html=True)
            st.markdown(
                f"<span style='color:{profit_col};font-weight:600'>{round(row['profit_margin']*100,2)}%</span>",
                unsafe_allow_html=True
            )
...
prev_col, page_col, next_col = st.columns([2,2,2])
...
with page_col:
    st.markdown(
        f"<div style='text-align:center;color:#9ca3af;font-size:15px;'>Page {page} of {total_pages}</div>",
        unsafe_allow_html=True
    )
