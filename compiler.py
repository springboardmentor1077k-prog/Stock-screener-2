def compile_dsl_to_sql(dsl_dict: dict):
    ...
    select_cols = "f.symbol, f.pe_ratio, f.market_cap, f.debt, f.revenue, f.ebitda, f.promoter_holding"
    ...
    CalculatedMetrics AS (
        SELECT company_id, quarter,
            CASE WHEN prev_revenue IS NULL OR prev_revenue = 0 THEN 0 
            ELSE ((revenue - prev_revenue) / prev_revenue) * 100 END as revenue_growth
        FROM GrowthData
    )
    """
    
    if time_filter:
        base_query = cte_query + f"SELECT {select_cols}, c.revenue_growth, c.quarter FROM fundamentals f JOIN CalculatedMetrics c ON f.symbol = c.company_id WHERE "
    else:
        base_query = f"SELECT {select_cols} FROM fundamentals f WHERE "
